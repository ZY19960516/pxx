...
Object.defineProperty(exports, "__esModule", { value: true });
let emoji_data_1 = __webpack_require__(0);
let emoji_panel_data_1 = __webpack_require__(3);
let parser_1 = __webpack_require__(4);
let EMOTION_SIZE = 40;
let emotionMap = {};
let emotionNames = [];
...
Component({
    ...
    lifetimes: {
        attached: function attached() {
            ...
            let areaWidth = systemInfo.windowWidth;
            let perLine = Math.floor((areaWidth - padding * 2) / 45);
            let extraPadding = Math.floor((areaWidth - padding * 2 - perLine * EMOTION_SIZE) / (perLine - 1));
            this.setData({
                perLine: perLine,
...
let parseEmoji = function parseEmoji(content) {
    let emojiIndexList = [];
    for (let k in emotionMap) {
    ...
    let lastText = content.substring(lastTextIndex);
    if (lastText) {
        newContentList.push({
            type: 1,
            content: lastText
